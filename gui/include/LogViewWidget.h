...
class LogViewWidget : public QWidget
{
    ...
    {
        gtp::TextCan can;
        std::optional<gtp::TextSelection> selection;
        std::vector<gtp::TextSelection> markedTexts;
    };
...
public:
    LogViewWidget(AbstractModel *model, std::vector<gtp::TextSelection> &markedTexts, QWidget *parent = nullptr);
    ~LogViewWidget();
    ...
    void retranslateUi();
    std::vector<gtp::TextSelection> &getMarkedTexts();
...
public slots:
    ...
    void addTextMark(const QString &text, const gtp::SectionColor &selColor);
    void removeTextMarks(const gtp::SectionColor &selColor);
...
protected:
    ...
    std::vector<gtp::TextSelection> findMarkedText(const gtp::TextCan &can);
    gtp::TextCan makeSelCanFromStrPos(const gtp::TextCan &can, int fromPos, int len);
    gtp::TextCan makeSelCanFromSelRect(const gtp::TextCan &can, const QRect &selRect);
...
private:
    ...
    std::vector<gtp::TextSelection> &m_markedTexts;
    ...
    std::vector<gtp::SectionColor> m_availableMarks;
