...
LogViewWidget::LogViewWidget(AbstractModel *model, std::vector<gtp::TextSelection> &markedTexts, QWidget *parent)
    : QWidget(parent),
      m_model(model),
...
std::vector<gtp::TextSelection> LogViewWidget::findMarkedText(const gtp::TextCan &can)
{
    std::vector<gtp::TextSelection> resVec;

    const auto findAndMarkFunc = [&, this](const QString &text, const gtp::SectionColor &color)
    {
        if (!text.isEmpty())
        {
...
void LogViewWidget::addTextMark(const QString &text, const gtp::SectionColor &selColor)
{
    m_markedTexts.emplace_back(gtp::TextCan(text), selColor);
    update();
    emit textMarkUpdated();
}

void LogViewWidget::removeTextMarks(const gtp::SectionColor &selColor)
{
    m_markedTexts.erase(
        std::remove_if(
            m_markedTexts.begin(),
...
std::vector<gtp::TextSelection> &LogViewWidget::getMarkedTexts()
{
