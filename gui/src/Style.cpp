...
void Style::clearStyle()
{
    ...
    const gtp::SectionColor nullSectionColor;
...
void Style::loadStyle(const QString &styleName)
{
    ...
    const auto loadColorSectionFromRoles =
        [&style](gtp::SectionColor &colorSec, QPalette::ColorRole fg, QPalette::ColorRole bg)
    {
        if (!colorSec.fg.isValid() || !colorSec.bg.isValid())
        {
            colorSec = gtp::SectionColor(style.m_palette.color(fg), style.m_palette.color(bg));
        }
    };
    ...
    const auto loadDefaultColorSection = [](gtp::SectionColor &colorSec, const QString &defFg, const QString &defBg)
    {
        if (!colorSec.fg.isValid() || !colorSec.bg.isValid())
        {
            colorSec.fg = defFg;
