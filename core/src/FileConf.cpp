...
void FileConf::fromJson(const rapidjson::Document &jDoc)
{
    ...
    if (const auto &hltIt = jDoc.FindMember("highlighters"); hltIt != jDoc.MemberEnd())
    {
        for (const auto &hlt : hltIt->value.GetArray())
        {
            ...
            hParam.color.bgColorName = utl::GetValueOpt<std::string>(hlt, "backColor").value_or("White");
            hParam.color.fgColorName = utl::GetValueOpt<std::string>(hlt, "textColor").value_or("Black");
...
rapidjson::Document FileConf::toJson() const
{
...

    {
        ...
        for (const auto &hlt : m_highlighterParams)
        {
            ...
            jHlt.AddMember("textColor", hlt.color.fgColorName, alloc);
            jHlt.AddMember("backColor", hlt.color.bgColorName, alloc);
