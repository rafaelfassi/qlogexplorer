...
{
    ColorParam() = default;
    ColorParam(const std::string &_fg, const std::string &_bg) : fgColorName(_fg), bgColorName(_bg) {}
    std::string fgColorName;
    std::string bgColorName;
};
inline bool operator==(const ColorParam &lhs, const ColorParam &rhs)
{
    return (lhs.fgColorName == rhs.fgColorName) && (lhs.bgColorName == rhs.bgColorName);
}
